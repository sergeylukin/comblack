...
if( ! function_exists( 'avia_woo_dynamic_css' ) )
{
	 ...
	 * @return string
	 */
	function avia_woo_dynamic_css( $output, array $color_set )
	{
		/*color sets*/
		foreach( $color_set as $key => $colors ) // iterates over the color sets: usually $key is either: header_color, main_color, footer_color, socket_color
		{
			...
			if( $key == '.main_color' )
			{
				...
				#top .avia-wc-notice-box$key .woocommerce-message a.button{
					background-color: $primary;
					color: $constant_font;
					border-color: $button_border;
				}
				#top .avia-wc-notice-box$key .woocommerce-message a.button:hover{
