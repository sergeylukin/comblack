...
Extras is one of the first premium Wordpress plugin for Elementor. It extends its capability with many unique widgets and extensions that make Elementor even more powerful.

= Copyright and licensing =

Copyright (C) 2017-2020 Munteanu M. Tudor II
...
* Added: Search Form — New filter 'elementor_extras/widgets/search-form/button/sr_text' for button screen reader text
* Fixed: Search Form — Taxonomy search returning empty results for other taxonomy filters present
* Fixed: Offcanvas — Page jump to top only on first open

= 2.2.39 =

* Added: Display Conditions — Post term condition
* Fixed: Posts — Duplicate posts on infinite scroll on front page when using static page
* Fixed: Toggle Element — Stacked alignment not working
* Tweak: Posts — Allow html in the no posts found message
* Tweak: Scroll Indicator – Missing default tooltip show event

= 2.2.38 =

* Fixed: Posts — Elementor compatibility with recent pagination fix
* Fixed: Posts — Duplicate content loading on infinite scroll
* Fixed: Search Form — Search returning empty results when using taxonomy filters with no terms selected
* Tweak: Posts — Custom pagination rewrite for widgets placed on single pages

= 2.2.37 =

* Added: Scroll Indicator — Allow scroll offset option for all skins
* Fixed: Gallery — Vertical spacing control not applying spacing when images are not linked
* Fixed: Posts — Pagination bug with WP 5.5
* Tweak: Search Form — Allow search without query

= 2.2.36 =

* Tweak: Revert support for Elementor Pro to 2.8.0
...
* Fixed: Swiper destroy on element remove fails with multiple instances on page
* Fixed: Slide Menu — Sibling submenu items showing behind current active submenu
* Fixed: Slide Menu – Removed default browser padding for ul
* Fixed: Search Form — Edge case where restrictions not being passed if page is loaded using back browser button

= 2.2.31 =

* Added: Toggle Element — Option to enable refreshing elements inside template when toggling
* Fixed: Toggle Element — Recalculate swipers inside templates
* Fixed: Ensure loop template data attributes
* Fixed: Gallery Slider — Compatibility with usage inside looped template
* Fixed: Circle Progress – Decimal point applied to all widget instances
* Fixed: Offcanvas — JS error on Offcanvas anchor navigation on links with dynamic tags enabled
* Tweak: Remove focus outline from swiper arrows
* Tweak: Circle Progress — Remove limit for max value when using absolute

= 2.2.30 =

* Fixed: Gallery Slider — Active thumbnail index for multiple widgets on same page
* Fixed: Gallery Slider — Remove buggy auto-height control for thumbnails carousel
* Fixed: Offcanvas — Bug with overlay and reveal animations on same page
* Fixed: Audio Player – Reset user agent padding on playlist
* Tweak: Posts Extra — Exclude em and rem from terms typography control

= 2.2.29 =

* Added: Gallery Slider — Possibility to add multiple caption types
* Added: Gallery Slider — Ken Burns effect option with. advanced controls
* Fixed: Gallery Slider — Missing preview captions text align control
* Fixed: Display Conditions — ACF Choice field check fails
* Tweak: Gallery Slider — Remove fade effect on thumbnails carousel
* Tweak: Gallery Slider — Condition image fit control to work only for slide effect

= 2.2.28 =

* Added: Toggle Element — Padding control for labels
* Added: Breadcrumbs — Control to disable structured data attributes
* Added: Gallery Slider, Gallery Extra, Switcher — Option to disable slideshow in lightbox
* Added: Posts — Wordpress filters for post and post wrapper classes
* Added: Video Player — Control to remove Chrome's download option in fullscreen mode
* Fixed: Gallery Slider, Gallery Extra, Random Image, Switcher — Missing lightbox captions
* Fixed: Toggle Element — Low CSS specificity for ul and li elements causing themes override
* Fixed: Popup — Exit intent not working correctly
* Tweak: Updated translations .pot file
...
* Added: Display Conditions — If no condition value is set check to see if field is set to any value
* Added: Gallery Slider — Options to set max. width and horizontal and vertical alignment for thumbnails carousel
* Added: Gallery Slider — Option to set custom height for preview slides
* Added: Popup — Option to disable refreshing frontend functionality for elements inside the popup
* Added: Offcanvas — Option to disable refreshing frontend functionality for elements inside the panel
* Added: Calendar — Typography control for header week days
* Added: Google Map — Option to set zoom level when navigating to a location
* Fixed: Bug where one swiper slider's arrows control all swiper sliders on the same page
* Fixed: Calendar — Separator style for header not working
* Fixed: Gallery — Instagram lightbox not working for my photos
* Fixed: Gallery — Instagram links bug for hashtag posts
* Fixed: Posts Extra — Filters with masonry enabled when two widgets on page breaks filtering
* Tweak: Circle Progress — Allow 0 value to be shown

= 2.2.24 =

* Added: Gallery Slider — Option to enable carousel on thumbnails
* Added: Posts Extra — More swiper settings
* Fixed: Circle Progress — Compatibility with display conditions
* Fixed: Posts Extra — Dynamic tag background styles
* Fixed: Don't include Elementor form widget in refreshable widgets due to reCaptcha reinitialisation uncaught error
* Fixed: Timeline — Date entrance animation on left right and overlay alignments

= 2.2.23 =

* Added: Popup — Margin control for close icon
* Fixed: Gallery Extra — Instagram compatibility with new API changes
* Fixed: Posts Extra — Carousel compatibility with Elementor 2.9.4 swiper.js changes
* Fixed: Posts Extra — Allow -1 as posts per page value for specific query source
* Fixed: Display Conditions — Use wp_timezone instead of timezone_string in case timezone is set as offset
* Tweak: Posts Extra — Filter post count style controls conditions
* Tweak: Posts Extra — Carousel remove deprecated Columns control
* Tweak: Google Map — Convert description control from text to WYSIWYG type

= 2.2.22 =

* Fixed: Timeline — Points and dates not showing when animate cards is on
* Fixed: Unfold — Bug error showing button since last update

= 2.2.21 =

* Added: Display Conditions — Current Date & Time condition
* Added: Timeline — New option to reverse position of cards
* Added: Text Divider — Margin control for the text
* Added: Toggle Element — Option to hide all elements by default
* Added: Timeline — Controls for overriding text colour in custom mode
* Fixed: EE widget indicator in editor panel font
* Fixed: Unfold — Removed elementor-button class from button which was adding a lot of padding
* Fixed: Unfold — Editor preview spacing in js template
* Fixed: Query control width
* Fixed: PHP error on updater transient data
* Fixed: Video Player — Stop play when played once in viewport
* Fixed: Timeline — Correct dates when using custom post skin with a template that uses Posts Extra
* Fixed: Tooltips — Tooltips not closing on mobile with touchstart event
* Fixed: Timeline — Entrance animation triggering on page load instead of scroll to element
* Fixed: Timeline — Query section not hidden if Source is Custom
* Fixed: Multiple SELECT2 controls width bug (missing css) after Elementor update
* Fixed: Gallery Extra — Masonry layout broken when using custom size values in manual mode
* Fixed: Posts Extra — Changing filters disrupts any Elementor entrance animations placed after widget
* Fixed: Posts Extra — Run ready triggers for section and columns on infinite scroll
* Fixed: Posts Extra — Refresh elements even if infinite scroll button is off
* Fixed: ACF fields not showing inside templates when ACF Extended plugin is active
* Tweak: Unfold — Hide button when there is no content or visible lines exceeds number of existing lines
* Tweak: Tooltips — Integrate Elementor breakpoints
* Tweak: Gallery Extra — Improved buggy responsive custom sizing method
* Revert: Display Conditions — Added time to Current Date conditions' date control

= 2.2.20 =

* Added: Display Conditions — 'Current User' condition
* Added: Display Conditions — Added time to Current Date conditions' date control
* Fixed: Some icon controls hidden due to Elementor changing label_block control attribute
* Fixed: Popup — Close button control conditions
* Fixed: Popup — Close icon not hiding on frontend

= 2.2.19 =

* Added: Popup — Option to set max number of times a non-persistent popup should open
* Fixed: Posts — Carousel compatibility with Elementor 2.9.0 swiper js upgrade
* Fixed: Switcher — Title direction on RTL
...
* Fixed: Popup — Date comparison fail for non-persistent interval setting
* Fixed: Tooltips — Tooltips empty in some cases after last update
* Fixed: Calendar — Support for ACF date time picker fields
* Tweak: Hotspots — Better css method for disabling pulse effect
* Tweak: Posts Extra — New "hide" icon for content parts positioning
* Tweak: Nicons

= 2.2.14 =

* Fixed: Audio Player — Edge case bug when stopping a player inside a section stuck with Elementor sticky setting
* Fixed: Switcher — Loader thickness not working in Firefox
* Fixed: Offcanvas — WPML not fetching content boxes repeater fields
* Fixed: Posts Extra — Masonry missing on frontend after last update
* Tweak: Tooltips — Use append method to add content to the tooltip

= 2.2.13 =

* Fixed: Posts Extra — Media left right alignment not working

= 2.2.12 =

* Added: Posts Extra — Post border radius control
* Fixed: Posts Extra — Safari layout bug on infinite scroll and masonry layout
* Fixed: Posts Extra — Carousel slides to scroll on tablet bug
* Fixed: Timeline — Date options not showing for custom post skin option
* Fixed: Sticky Elements — Force sticky sections to flex align to start for Safari flex stretch bug

= 2.2.11 =

* Added: Posts Extra — Infinite scroll fields for WPML
* Fixed: Breadcrumbs — Better support for post type archive links on archive pages
* Fixed: Posts Extra — Flexbox causing horizontal media layout issue on mobile
* Fixed: Circle Progress — Compatibility with wordpress 5.3
* Tweak: Offcanvas — Mode container background colour control from site settings to widget settings

= 2.2.10 =

* Added: Popup — Option to open url inside popup
* Fixed: Breadcrumbs — Check for existing non built-in custom post types before iteration
* Fixed: Display Conditions — ACF Date support for custom locale
* Fixed: Calendar — ACF Date support for custom locale
* Fixed: Switcher — Description not available for translation
* Fixed: Gallery — Safari backface-visibility bug

= 2.2.9 =

* Added: Posts Extra — Align media vertically on horizontal post layouts
* Fixed: Breadcrumbs — Text separator not displaying
* Tweak: Posts Extra — Default horizontal layout width control to 30

= 2.2.8 =

* Added: Breadcrumbs — Granular controls for crumbs settings on archive and single pages
* Fixed: Posts Extra — Moved images loaded container selector to ajax response
* Fixed: Posts Extra — Show all filters option not working
* Fixed: Posts Extra — Infinite scroll pagination html output rendering more links than needed
* Fixed: PHP notice for licensing admin notification
* Fixed: Switcher — Border radius control CSS selector
* Tweak: Posts Extra — Cleanup on pagination html output needed for infinite scroll

= 2.2.7 =

* Fixed: Compatibility with third-party plugin
* Fixed: Offcanvas — Edge case bug when calculating anchor position
* Fixed: Breadcrumbs — Notices
* Fixed: Posts Extra — Overlapping items on infinite scroll when filters active

= 2.2.6 =

* Added: Posts Extra — Global 'elementor_extras/widgets/posts/link' filter
* Added: Offcanvas — Typography control for content boxes
* Fixed: Posts Extra — Infinite Scroll stopping before all posts load
* Fixed: Offcanvas — Panel background selector
* Fixed: Offcanvas — Page jump when navigating to anchor
* Fixed: Posts Extra — Edge case js error in nouislider script
* Fixed: Templates Control — Restrict global query override for templates inside loops
* Fixed: Slide Menu — Nav menu item classes don't work when coming from template control
* Fixed: Breadcrumbs — Wrong order for hierarchical taxonomies
* Fixed: Posts Extra — Flicker when performing infinite scroll on filtered posts
* Fixed: Sticky Elements — Additional padding in some cases below sticky element
* Fixed: Display Conditions — Missing day of week condition
* Fixed: Hotspots — Better handling of icon vertical alignment in css

= 2.2.5 =

* Added: Toggle Element — Control to hide items without any content
* Added: Popup — Support for usage within Posts Extra custom post skin
* Added: Audio Player — Option to disable preloading of audio files
* Added: Calendar — Search custom fields by field name
* Added: Offcanvas — Options in widgets' settings to fix page jump issue
* Fixed: Posts Extra — Certain actions not working
* Fixed: Calendar — Pods controls not fetching fields
* Fixed: Calendar — Toolset controls not fetching fields
* Fixed: Query Control — Return only matching queries for pods and toolset
* Fixed: Calendar — Placeholder functionality
* Fixed: Offcanvas — Edge case where css width selectors get overridden by defaults
* Tweak: Posts Extra – Default css order value for parts and areas in frontend css
* Tweak: Popup — Replaced multiple template selection control with single control. IMPORTANT: Settings review required!
* Tweak: Breadcrumbs — Refactor widget code

= 2.2.4 =

* Added: Dynamic background image support for custom post skin
* Added: Breadcrumbs — Filter for home breadcrumb link
* Fixed: Timeline — Wrong card content selector for transition control
* Fixed: Offcanvas — Broken selectors after last offcanvas update
* Tweak: Posts Extra — Remove default border around posts

= 2.2.3 =

* Added: Offcanvas — Compatibility with templates in loops in Posts Extra and other plugins
* Added: Posts Extra — Refresh all widgets scripts on appended infinite scroll items
* Fixed: Posts Extra — Button style gone when placed in linked media area
* Fixed: Sticky Elements — Bug when sticky section inside page body within theme builder header template
* Fixed: Scroll Indicator — Tooltips not working with bullets skin

= 2.2.2 =

* Fixed: Scroll Indicator — Bug when tooltips disabled on bullets skin
* Fixed: Timeline — Option to hide thumbnail not working in posts mode
* Fixed: Breadcrumbs — Bug with before parent category filters
* Fixed: Bug with third party elementor addon that enqueue editor scripts on plugin load
* Fixed: Posts — Default metas control to include price for previous saved settings
* Added: Search Form — Compatibility with Relevanssi search results on custom templates with search form id condition
* Added: Timeline — Option to set a dynamic tag instead of date, title and excerpt in posts mode
* Added: Timeline — Option to set HTML element for title in posts mode
* Added: Posts Extra — Filter for the media area link
* Added: Posts Extra — Option to open media and button links in new tab
* Added: Table — WPML translation of header cells text
* Tweak: Sticky Elements — Additional listeners for recalculating sticky
* Tweak: Posts Extra — Less restrictive css selector for the woocommerce add to cart button

= 2.2.1 =

* Added: Scroll Indicator — Option to offset start and end positions
* Added: Posts Extra — Responsive media width control
* Fixed: Posts Extra — Missing custom field order options
* Fixed: Posts Extra — Avatar meta showing even if removed from metas list
* Fixed: Display Conditions — ACF Choice fails for radio field
* Fixed: Posts Extra — Bug with left aligned media flex width on mobile

= 2.2.0 =

* Added: Google Map — Route travel mode options
* Added: Global Tooltips — Compatibility with usage inside post skin templates for Posts Extra
* Added: Global Tooltips — z-index control in both element and global settings
* Added: Display Conditions — Term archive condition
* Added: Display Conditions — ACF conditions
* Added: Display Conditions — POST and GET variable conditions
* Added: Timeline — Custom Post Skin via Elementor templates
* Added: Timeline — Replaced default query controls with related query controls
* Added: Timeline — Trim custom excerpt control
* Added: Timeline — Custom post skin
* Added: Posts Extra — Date formatting controls
* Added: Posts Extra — Actions and filters
* Added: Calendar — Actions and filters
* Added: Devices — Actions
* Added: Breadcrumbs — Actions and filters
* Added: Scroll Indicator — Custom html tags for title and subtitle in list skin
* Added: Custom implementation for Template controls
* Fixed: Toggle Element — Support for slick slider based widgets inside elements
* Fixed: Switcher — Missing image link control
* Fixed: Timeline — Markup improvement to allow for simpler border radius around cards
* Fixed: Timeline — Custom icon replaces global if global is set
* Fixed: Breadcrumbs — Separator vertical alignment bug
* Fixed: Breadcrumbs — Icon migration markup issue
* Fixed: Posts Extra — Update elementor nerd box classes
* Fixed: Posts Extra — ACF fields for custom skins used on archive pages
* Fixed: Posts Extra — Wrong access type for _query property
* Fixed: Posts Extra — Price conditions issue for area and editor panel section display
* Fixed: Posts Extra — Author and avatar metas additional controls conditions
* Fixed: Posts Extra — Removed infinite scroll pages loaded limit
* Fixed: Posts Extra — Handle multiple paginations control condition bug
* Fixed: Scroll Indicator — Custom height control condition
* Fixed: Popup — Compatibility with usage inside post skin templates for Posts Extra
* Fixed: Buttons — Flip effect back is hidden during transition
* Tweak: Breadcrumbs — Remove useless icon and icon class filters
* Tweak: Posts Extra — Update button text filters to include /posts/
* Tweak: Posts Extra — Default taxonomy order for filters
