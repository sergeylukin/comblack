...
if( ! class_exists( 'aviaPostCssManagement' ) )
{
	...
	class aviaPostCssManagement
	{
		 ...
		 */
		public function handler_wp_save_post( $post_id, WP_Post $post, $update )
		{
			global $post;
		...
		public function css_files_directory()
		{
			...
			{
				$this->css_files_directory = apply_filters( 'avf_post_css_management_files_directory', 'avia_posts_css' );
			}
		...
		public function get_css_rules_from_content( $content, $post_id, $destination = 'file' )
		{
			...
			$this->css_rules_from_shortcodes( $shortcodes );
		 ...
		 * @param array $shortcodes
		 */
		protected function css_rules_from_shortcodes( array $shortcodes )
		{
			foreach( $shortcodes as $shortcode )
			{
				$item = false;
				...
				if( ! empty( $result ) )
				{
					$rules = $result['element_styling']->get_style_rules();
					if( ! empty( $rules ) )
					{
