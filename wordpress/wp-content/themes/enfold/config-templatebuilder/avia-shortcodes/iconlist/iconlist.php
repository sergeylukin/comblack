...
if( ! class_exists( 'avia_sc_iconlist' ) )
{
	class avia_sc_iconlist extends aviaShortcodeTemplate
	{
		...
		protected function get_element_styles( array $args )
		{
			...
			{
				$element_styling->add_styles( 'item-title', array( 'font-size' => $atts['custom_title_size'] . 'px' ) );
			...
			if( $atts['custom_content_size'] != '' )
			{
				$element_styling->add_styles( 'item-content', array( 'font-size' => $atts['custom_content_size'] . 'px' ) );
			}
...
//			$this->subitem_inline_styles .= $element_styling->get_style_tag( $element_id, 'rules_only' );
			...
			if( ! empty( $atts['title'] ) )
			{
				$output .=					"<{$title_el} class='av_iconlist_title {$iconlist_title} {$css} {$this->title_class} {$av_title_font_classes}' {$markup_title}>{$atts['title']}</{$title_el}>";
			}
			$output .=                  '</header>';

			$output .=                  "<div class='iconlist_content {$this->content_class} {$av_font_classes}' {$markup_content}>" . ShortcodeHelper::avia_apply_autop( ShortcodeHelper::avia_remove_autop( $content ) ) . '</div>';
			$output .=              '</div>';
