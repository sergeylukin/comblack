	...
	class avia_sc_heading extends aviaShortcodeTemplate
	{
		...
		protected function register_dynamic_templates()
		{
			...
			$c = array(
						...
						array(
							'name'		=> __( 'Custom Seperator Line Color', 'avia_framework' ),
							'desc'		=> __( 'Select a custom color for the seperator line beside the title here (only available on some styles)', 'avia_framework' ),
							'id'		=> 'seperator_color',
							'type'		=> 'colorpicker',
							'std'		=> '',
							'rgba'		=> true,
			...
			$c = array(
						...
						array(
							...
							'desc' 	=> __( 'Set the bottom padding for the element in pixel', 'avia_framework' ),
							'id' 	=> 'padding',
							'type' 	=> 'select',
							'std'	=> '10',
							'lockable'	=> true,
						...
						array(
							'name' 	=> __( 'Headline Text Padding', 'avia_framework' ),
							'desc'	=> __( 'Set a distance around the headline text. Leave empty for default value. Both pixel and &percnt; based values are accepted. eg: 30px, 5&percnt;. px is used as default unit.', 'avia_framework' ),
							'id' 	=> 'headline_padding',
							'type' 	=> 'multi_input',
							'std' 	=> '',
							'sync' 	=> true,
		...
		protected function get_element_styles( array $args )
		{
			...
			{
				$margin_calc = AviaHelper::multi_value_result_lockable( $atts['margin'] );
				$element_styling->add_styles( 'container', array( 'margin' => $margin_calc['fill_with_0_style'] ) );
			}

			if( ! empty( $atts['headline_padding'] ) )
			{
				$margin_calc = AviaHelper::multi_value_result_lockable( $atts['headline_padding'] );
				$element_styling->add_styles( 'heading-text', array( 'padding' => $margin_calc['fill_with_0_style'] ) );
			}

			$element_styling->add_styles( 'container', array( 'padding-bottom' => $atts['padding'] . 'px' ) );
			...
			if( ! empty( $atts['style'] ) && ! empty( $atts['size'] ) )
			{
				...
				$element_styling->add_styles( 'container', array( 'font-size' => $atts['size'] ) );
			...
			if( ! empty( $atts['subheading_active'] ) )
			{
				...
				if( ! empty( $atts['style'] ) && ! empty( $content ) )
				{
					$element_styling->add_styles( 'subheading', array( 'font-size' => $atts['subheading_size'] . 'px' ) );
				}
			...
			if( $atts['style'] == 'blockquote elegant-quote elegant-centered' )
			{
				if( $atts['show_icon'] == 'custom_icon' && $atts['icon'] !== '' )
				{
					...
					{
						$element_styling->add_styles( 'heading-icon', array( 'font-size' => $atts['icon_size'] . 'px' ) );
					}
					...
					if( is_numeric( $atts['icon_padding'] ) )
					{
						$element_styling->add_styles( 'heading-icon', array( 'padding-bottom' => $atts['icon_padding'] . 'px' ) );
					}
				}
			}
			...
			$selectors = array(
						...
						'heading-text'		=> ".av-special-heading.{$element_id} .av-special-heading-tag",
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			if( ! empty( $link ) )
			{
				$link_before .= '<a class="av-heading-link" href="' . AviaHelper::get_url( $link ) . '"' . AviaHelper::get_link_target( $link_target ) . '>';
				$link_after .= '</a>';
			}
			...
			if( $style == 'blockquote elegant-quote elegant-centered' )
			{
				...
				if( $show_icon == 'custom_icon' && $icon !== '' )
				{
					...
					$output_before = "<span class='heading-char avia-font-{$font} {$av_font_classes_1}' {$display_char}></span>";
				}
			...
			if( ! empty( $style ) && ! empty( $subheading_active ) && ! empty( $content ) )
			{
				...
				$content = "<div class='{$subheading_class} {$av_font_classes}'>" . ShortcodeHelper::avia_apply_autop( ShortcodeHelper::avia_remove_autop( $content ) ) . '</div>';
			...
			$output .= 		$after;
			$output .= 		"<div class='special-heading-border'><div class='special-heading-inner-border'></div></div>";
			$output .= '</div>';
