...
if( ! class_exists( 'avia_mailchimp_widget' ) )
{
	class avia_mailchimp_widget extends WP_Widget
	{
		...
		{
			$widget_ops = array(
							'classname'		=> 'avia_mailchimp_widget',
							'description'	=> __( 'A widget that displays a Mailchimp newsletter signup form', 'avia_framework' )
						);

			// @since 4.8.7: https://kriesi.at/support/topic/php-error-with-every-update/
			$theme = wp_get_theme();
			$title = $theme->get( 'Name' ) . ' ' . __( 'Mailchimp Newsletter Signup', 'avia_framework' );

			parent::__construct( 'avia_mailchimp_widget', $title, $widget_ops );
		}
